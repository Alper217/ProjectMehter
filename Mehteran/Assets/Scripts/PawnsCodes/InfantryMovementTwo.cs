...
public class InfantryMovementTwo : MonoBehaviour
{
    ...
    void HighlightAdjacentTiles()
    {
        ...
        foreach (var col in adjacentColliders)
        {
            ...
            // Sað ve sol pozisyonlarý kontrol etmek için fark vektörünü hesapla
            Vector3 direction = col.transform.position - currentHex.transform.position;
            direction.y = 0; // Yüksekliði (y ekseni) göz ardý et

            // Sadece sað ve sol hexagonlara izin ver (yaklaþýk 60 ve 120 derece arasýndaki yönler)
            if (Vector3.Angle(Vector3.right, direction) < 90 || Vector3.Angle(-Vector3.right, direction) < 90)
            {

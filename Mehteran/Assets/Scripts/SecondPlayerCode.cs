...
public class SecondPlayerCode : MonoBehaviour
{
    ...
    public Material sideOneHexMaterial; // Pembe bölge
    // HEX ALANI
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hit))
            {
                ...
                else if (isSelected && hit.collider.CompareTag("HexTileEnemy") && adjacentHexes.Contains(hit.collider.gameObject))
                {
                    MoveToTileCenter(hit.collider.gameObject);
                }
                else if (isSelected && hit.collider.CompareTag("HexTile") && enemyHexes.Contains(hit.collider.gameObject))
                {
    ...
    void CaptureEnemyTile(GameObject enemyTile)
    {
        ...
        if (renderer != null)
        {
            renderer.material = sideOneHexMaterial; // HexTile için varsayýlan mavi materyale ayarla
            enemyTile.tag = "HexTileEnemy"; // DEÐÝÞTÝ
        }
    ...
    void UpdateCurrentHex()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit) && hit.collider.CompareTag("HexTileEnemy"))
        {
    ...
    void HighlightAdjacentTiles()
    {
        ...
        foreach (var col in adjacentColliders)
        {
            ...
            if (renderer != null)
            {
                if (col.CompareTag("HexTileEnemy"))
                {
                ...
                else if (col.CompareTag("HexTile"))
                {
                    if (!originalMaterials.ContainsKey(col.gameObject))
                    {
                        originalMaterials[col.gameObject] = renderer.material;
